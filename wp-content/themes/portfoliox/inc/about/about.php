...
function portfoliox_new_optins_texts()
{
	...
	$portfoliox_version = $portfoliox_theme->get('Version');
	global $pagenow;
	if (get_option('portfolioxfit' . $portfoliox_version) || $pagenow == 'themes.php') {
		return;
	}
...
function portfoliox_new_optins_texts_init()
{
	...
	if (isset($_GET['xbnotice']) && $_GET['xbnotice'] == 1) {
		delete_option('portfolioxfit5');
		update_option('portfolioxfit' . $portfoliox_version, 1);
	}
}
